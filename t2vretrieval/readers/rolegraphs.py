...
class RoleGraphDataset(t2vretrieval.readers.mpdata.MPDataset):
        ...
        if self.load_video_first:
            ...
            for name in self.names:
                ...
                attn_fts, attn_len = self.pad_or_trim_feature(
                    attn_fts, self.max_attn_len, trim_type='select')
    ...
    def pad_or_trim_feature(self, attn_ft, max_attn_len, trim_type='top'):
        ...
        # trim
            ...
            if trim_type == 'top':
                ...
                idxs = np.round(np.linspace(
                    0, seq_len-1, max_attn_len)).astype(np.int32)
                new_ft = attn_ft[idxs]
    ...
    def get_caption_outs(self, out, sent, graph):
        ...
        edges = []
        out['node_roles'] = np.zeros(
            (self.num_verbs + self.num_nouns, ), np.int32)
        ...
        node_idx = 1
        out['verb_masks'] = np.zeros(
            (self.num_verbs, self.max_words_in_sent), np.bool)
        ...
        out['noun_masks'] = np.zeros(
            (self.num_nouns, self.max_words_in_sent), np.bool)
        for knode, vnode in graph_nodes.items():
            ...
            if vnode['role'] not in ['ROOT', 'V'] and np.min(vnode['spans']) < self.max_words_in_sent:
                ...
                out['node_roles'][node_idx -
                                  1] = self.role2int.get(vnode['role'], self.role2int['NOUN'])
                node_idx += 1
