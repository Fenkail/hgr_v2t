...
def create_role_graph_data(srl_data):
    ...
    root_name = 'ROOT'
    graph_nodes[root_name] = {'words': words, 'spans': list(
        range(0, len(words))), 'role': 'ROOT'}
    ...
    return graph_nodes, graph _edges
...
for sent, graph in sent2graph.items():
    ...
    if len(nodes) == 1:
        ...
        for w in doc.noun_chunks:
            ...
            nodes[node_name]['words'] = [
                doc[j].text for j in nodes[node_name]['spans']]
            node_idx += 1
        if len(nodes) == 1:
            for w in doc:
                node_name = str(node_idx)
